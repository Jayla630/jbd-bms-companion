...
test('parseBasicInfo.balance:黄金响应无均衡 → false', () => {
  assert.equal(parseBasicInfo(GOLDEN_BASIC).balance, false);
});

test('parseBasicInfo.balance:1、3 串均衡中(数据区偏移12 = 0x0005)→ true', () => {
  const d = Array.from(GOLDEN_BASIC.slice(4, 4 + GOLDEN_BASIC[3]));
  d[12] = 0x00; d[13] = 0x05;
  const frame = makeResponse(REG_BASIC, 0x00, d);
